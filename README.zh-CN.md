...
**`config.yaml` 在 v0.1.1 之前是装饰品。** 十一个字段里有十个不被任何 Skill 读取。
现在已经接通，但只有 `mode`、`concurrency.verify_file_fingerprints` 和两个
`review.*` 键在实际运行中被观察到使用；其余仍属于「意图说明」。

**规模从未测过。** `tests/` 里所有运行都针对一个四文件的项目。当「读完范围内的每个
文件」意味着一千个文件时，设计阶段会怎样，没有任何数据。

**`paths.root` 目前仍然改不动任何东西。** 这个键有文档、Skill 现在也会读 config，
但 `.ho/` 在多处是字面写死的。改它还不会生效。
